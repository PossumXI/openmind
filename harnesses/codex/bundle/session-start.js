...
function statePath() {
  return join19(homedir12(), ".deeplake", "notifications-state.json");
}
function readState() {
  try {
...
function writeState(state) {
  ...
  mkdirSync11(join19(home, ".deeplake"), { recursive: true, mode: 448 });
  const tmp = `${path}.${process.pid}.tmp`;
...
function tryClaim(n) {
  const home = resolve4(homedir12());
  const claimsDir = join19(home, ".deeplake", "notifications-claims");
  try {
...
function claimPathFor(claimsDir, n) {
  ...
  return join19(claimsDir, `${safeId}-${keyHash}`);
}
...
function cacheFilePath() {
  return join20(homedir13(), ".deeplake", "hivemind-stats-cache.json");
}
...
import { existsSync as existsSync12, readdirSync as readdirSync3, statSync as statSync4, writeFileSync as writeFileSync12, readFileSync as readFileSync15, openSync as openSync4, readSync, closeSync as closeSync4 } from "node:fs";
...
var PROJECTS_DIR = () => join21(homedir14(), ".claude", "projects");
var STATE_FILE = () => join21(homedir14(), ".claude", ".hivemind_brief_state.json");
...
function mineLocal(cutoff) {
  ...
  for (const d of projDirs) {
    ...
    let files;
    try {
      files = readdirSync3(join21(base, d));
    } catch {
      continue;
    }
    for (const f of files) {
      ...
      if (Date.now() > deadline2)
        break;
      const s = loadLocalSession(join21(base, d, f), cutoff);
      if (s)
        out.push(s);
    }
  }
...
import { readFileSync as readFileSync16, writeFileSync as writeFileSync13, writeSync, mkdirSync as mkdirSync13, existsSync as existsSync13, unlinkSync as unlinkSync7, openSync as openSync5, closeSync as closeSync5, statSync as statSync5 } from "node:fs";
import { homedir as homedir15 } from "node:os";
import { join as join22 } from "node:path";
var STATE_DIR = join22(homedir15(), ".claude", "hooks", "summary-state");
var YIELD_BUF = new Int32Array(new SharedArrayBuffer(4));
function statePath2(sessionId) {
  return join22(STATE_DIR, `${sessionId}.json`);
}
function endedMarkerPath(sessionId) {
  return join22(STATE_DIR, `${sessionId}.ended`);
}
function ownerPath(sessionId) {
  return join22(STATE_DIR, `${sessionId}.owner`);
}
...
function isSessionLive(sessionId, withinMs = activeWindowMs()) {
  ...
  if (owner === "dead")
    return false;
  try {
    const mtimeMs = statSync5(statePath2(sessionId)).mtimeMs;
...
function statsFilePath() {
  return join23(homedir16(), ".deeplake", "usage-stats.jsonl");
}
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join23(homedir16(), ".claude", "skills");
  if (!existsSync14(dir))
    return 0;
...
async function main() {
  ...
  {
    const setupScript = join24(__bundleDir, "session-start-setup.js");
