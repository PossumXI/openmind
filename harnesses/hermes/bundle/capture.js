...
function resolveCaptureConfig(cwd, log18) {
  ...
  if (!resolved.collect) {
    log18(`capture disabled for cwd=${effCwd} via ${resolved.found?.path}`);
    return null;
  }
...
function endedMarkerPath(sessionId) {
  return join11(STATE_DIR, `${sessionId}.ended`);
}
function ownerPath(sessionId) {
  return join11(STATE_DIR, `${sessionId}.owner`);
}
function procInfo(pid) {
  try {
    const s = readFileSync8(`/proc/${pid}/stat`, "utf-8");
...
function readOwner(sessionId) {
  try {
    return JSON.parse(readFileSync8(ownerPath(sessionId), "utf-8"));
  } catch {
    return null;
  }
}
...
function isSessionLive(sessionId, withinMs = activeWindowMs()) {
  if (existsSync6(endedMarkerPath(sessionId)))
    return false;
  ...
  if (owner === "dead")
    return false;
  try {
    const mtimeMs = statSync3(statePath(sessionId)).mtimeMs;
...
// dist/src/hooks/hermes/capture.js
import { closeSync as closeSync7, openSync as openSync7 } from "node:fs";
...
import { closeSync as closeSync5, mkdirSync as mkdirSync15, openSync as openSync5, readFileSync as readFileSync15, unlinkSync as unlinkSync8, writeFileSync as writeFileSync11 } from "node:fs";
import { createHash as createHash2 } from "node:crypto";
import { join as join25, resolve as resolve5 } from "node:path";
import { homedir as homedir17 } from "node:os";
var log6 = (msg) => log("notifications-state", msg);
function statePath3() {
  return join25(homedir17(), ".deeplake", "notifications-state.json");
}
function readState3() {
  try {
    const raw = readFileSync15(statePath3(), "utf-8");
    const parsed = JSON.parse(raw);
...
function writeState3(state) {
  const path2 = statePath3();
  const home = resolve5(homedir17());
  if (!isPathInsideHome(path2, home)) {
    throw new Error(`notifications-state write blocked: ${path2} is outside ${home}`);
  }
  mkdirSync15(join25(home, ".deeplake"), { recursive: true, mode: 448 });
  const tmp = `${path2}.${process.pid}.tmp`;
  writeFileSync11(tmp, JSON.stringify(state, null, 2), { mode: 384 });
  renameAtomic(tmp, path2);
}
...
function tryClaim(n) {
  ...
  try {
    mkdirSync15(claimsDir, { recursive: true, mode: 448 });
  } catch (e) {
  ...
  try {
    const fd = openSync5(claimPath, "wx", 384);
    closeSync5(fd);
    return true;
  } catch (e) {
    if (e?.code === "EEXIST")
...
function claimPathFor(claimsDir, n) {
  const keyHash = createHash2("sha256").update(JSON.stringify(n.dedupKey)).digest("hex").slice(0, 12);
...
import { existsSync as existsSync12, mkdirSync as mkdirSync16, readFileSync as readFileSync16, writeFileSync as writeFileSync12 } from "node:fs";
...
function cacheFilePath() {
  return join26(homedir18(), ".deeplake", "hivemind-stats-cache.json");
}
...
function writeCache(scopeKey, data) {
  try {
    mkdirSync16(dirname10(cacheFilePath()), { recursive: true });
    const body = { fetchedAt: Date.now(), scopeKey, data };
    writeFileSync12(cacheFilePath(), JSON.stringify(body), "utf-8");
  } catch (e) {
...
import { existsSync as existsSync13, readdirSync as readdirSync2, statSync as statSync5, writeFileSync as writeFileSync13, readFileSync as readFileSync17, openSync as openSync6, readSync, closeSync as closeSync6 } from "node:fs";
...
var PROJECTS_DIR = () => join27(homedir19(), ".claude", "projects");
var STATE_FILE = () => join27(homedir19(), ".claude", ".hivemind_brief_state.json");
...
function hasState() {
  return existsSync13(STATE_FILE());
}
function lastBriefMs() {
  try {
    if (!existsSync13(STATE_FILE()))
      return null;
...
function writeState4(sessionsScanned, isFirstRun) {
  try {
    writeFileSync13(STATE_FILE(), JSON.stringify({
...
function readHeadTail(path2, bytes) {
  let fd = null;
  try {
...
function loadLocalSession(path2, cutoff) {
  let mtime;
  try {
    mtime = statSync5(path2).mtime;
  } catch {
    return null;
  }
  if (mtime < cutoff)
    return null;
  const ht = readHeadTail(path2, HEAD_TAIL_BYTES);
  if (!ht)
    return null;
  ...
  if (last.ts < cutoff)
    return null;
  const projDirName = path2.split(/[/\\]/).slice(-2, -1)[0] ?? "unknown";
  return {
    firstTs: first.ts,
    lastTs: last.ts,
...
function mineLocal(cutoff) {
  ...
  for (const d of projDirs) {
    ...
    let files;
    try {
      files = readdirSync2(join27(base, d));
    } catch {
      continue;
    }
...
async function pickColdStartBrief(creds) {
  try {
    ...
    writeState4(sessions.length, !hadState);
...
function sessionIdFromSummaryPath(path2) {
...
function excludeActiveSessions(rows, currentSessionId, isLive = isSessionLive) {
  return rows.filter((row) => {
    ...
    if (!path2)
      return true;
    const sid = sessionIdFromSummaryPath(path2);
    if (!sid)
      return true;
...
function selectRealSummaries(rows, lookback = LOOKBACK) {
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      sid: sessionIdFromSummaryPath(path2)
    });
...
import { appendFileSync as appendFileSync5, existsSync as existsSync14, mkdirSync as mkdirSync17, readFileSync as readFileSync18, readdirSync as readdirSync3 } from "node:fs";
...
function statsFilePath() {
  return join28(homedir20(), ".deeplake", "usage-stats.jsonl");
}
function readUsageRecords() {
  try {
    if (!existsSync14(statsFilePath()))
      return [];
    const raw = readFileSync18(statsFilePath(), "utf-8");
    const out = [];
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join28(homedir20(), ".claude", "skills");
  if (!existsSync14(dir))
    return 0;
...
import { readFileSync as readFileSync19, writeFileSync as writeFileSync14, mkdirSync as mkdirSync18 } from "node:fs";
import { join as join29 } from "node:path";
import { homedir as homedir21 } from "node:os";
import { randomUUID as randomUUID3 } from "node:crypto";

// dist/src/hooks/hermes/capture.js
var log17 = (msg) => log("hermes-capture", msg);
async function maybeEmitSessionNotifications(sessionId) {
  try {
    if (!sessionId)
      return;
    const sentinel = join30(dirname12(sessionEventCachePath(sessionId)), `.notified-${sessionId}`);
    try {
      closeSync7(openSync7(sentinel, "wx"));
    } catch {
      return;
    }
    let notified = [];
    ...
    if (context) {
      process.stdout.write(JSON.stringify({ context }));
      log17(`notifications: delivered ${notified.length} via pre_llm_call context`);
    }
  } catch (e) {
    log17(`notification delivery failed: ${e instanceof Error ? e.message : String(e)}`);
  }
}
function resolveEmbedDaemonPath() {
  return join30(dirname12(fileURLToPath4(import.meta.url)), "embeddings", "embed-daemon.js");
}
var __bundleDir = dirname12(fileURLToPath4(import.meta.url));
...
async function main() {
  ...
  if (!isHivemindPluginEnabled()) {
    log17("plugin disabled, skipping capture");
    return;
  }
  const input = await readStdin();
  const config = resolveCaptureConfig(input.cwd ?? process.cwd(), log17);
  if (!config)
    return;
  ...
  if (event === "pre_llm_call") {
    ...
    if (!prompt) {
      log17(`pre_llm_call: no prompt found in extra`);
      return;
    }
    log17(`user session=${sessionId}`);
    ...
    log17(`tool=${input.tool_name} session=${sessionId}`);
    entry = {
    ...
    if (!text) {
      log17(`post_llm_call: no response found in extra`);
      return;
    }
    log17(`assistant session=${sessionId}`);
    entry = { id: crypto.randomUUID(), ...meta, type: "assistant_message", content: text };
  } else {
    log17(`unknown/unhandled event: ${event}, skipping`);
    return;
  }
  const sessionPath = buildSessionPath(config, sessionId);
  const line = redactSecrets(JSON.stringify(entry));
  log17(`writing to ${sessionPath}`);
  const projectName = projectNameFromCwd(cwd);
  ...
  try {
    ...
    if (e.message?.includes("permission denied") || e.message?.includes("does not exist")) {
      log17("table missing, creating and retrying");
...
function maybeTriggerPeriodicSummary(sessionId, cwd, config) {
  ...
  try {
    ...
    if (!tryAcquireLock(sessionId)) {
      log17(`periodic trigger suppressed (lock held) session=${sessionId}`);
      return;
    }
    ...
    try {
      ...
      log17(`periodic spawn failed: ${e.message}`);
      try {
    ...
    log17(`periodic trigger error: ${e.message}`);
  }
}
main().catch((e) => {
