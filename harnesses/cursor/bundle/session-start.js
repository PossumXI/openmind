...
import { join as join24, resolve as resolve5 } from "node:path";
import { homedir as homedir13 } from "node:os";
var log7 = (msg) => log("notifications-state", msg);
function statePath() {
  return join24(homedir13(), ".deeplake", "notifications-state.json");
}
function readState() {
  try {
...
function writeState(state) {
  ...
  mkdirSync14(join24(home, ".deeplake"), { recursive: true, mode: 448 });
  const tmp = `${path}.${process.pid}.tmp`;
...
function tryClaim(n) {
  const home = resolve5(homedir13());
  const claimsDir = join24(home, ".deeplake", "notifications-claims");
  try {
...
function claimPathFor(claimsDir, n) {
  ...
  return join24(claimsDir, `${safeId}-${keyHash}`);
}
...
function cacheFilePath() {
  return join25(homedir14(), ".deeplake", "hivemind-stats-cache.json");
}
...
import { existsSync as existsSync16, readdirSync as readdirSync3, statSync as statSync4, writeFileSync as writeFileSync14, readFileSync as readFileSync17, openSync as openSync4, readSync, closeSync as closeSync4 } from "node:fs";
...
var PROJECTS_DIR = () => join26(homedir15(), ".claude", "projects");
var STATE_FILE = () => join26(homedir15(), ".claude", ".hivemind_brief_state.json");
...
function mineLocal(cutoff) {
  ...
  for (const d of projDirs) {
    ...
    let files;
    try {
      files = readdirSync3(join26(base, d));
    } catch {
      continue;
    }
    for (const f of files) {
      ...
      if (Date.now() > deadline)
        break;
      const s = loadLocalSession(join26(base, d, f), cutoff);
      if (s)
        out.push(s);
    }
  }
...
import { readFileSync as readFileSync18, writeFileSync as writeFileSync15, writeSync, mkdirSync as mkdirSync16, existsSync as existsSync17, unlinkSync as unlinkSync7, openSync as openSync5, closeSync as closeSync5, statSync as statSync5 } from "node:fs";
import { homedir as homedir16 } from "node:os";
import { join as join27 } from "node:path";
var STATE_DIR = join27(homedir16(), ".claude", "hooks", "summary-state");
...
function statsFilePath() {
  return join28(homedir17(), ".deeplake", "usage-stats.jsonl");
}
...
function countUserGeneratedSkills(userName) {
  if (!userName)
    return 0;
  const dir = join28(homedir17(), ".claude", "skills");
  if (!existsSync18(dir))
    return 0;
